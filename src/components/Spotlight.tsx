...
import { useLaunchpadApps } from "~/configs/launchpad";
...
export default function Spotlight({
  ...
  const apps = useApps();
  const launchpadApps = useLaunchpadApps();
  ...
  const [allApps, setAllApps] = useState<{ [key: string]: (LaunchpadData | AppsData)[] }>({
    app: [],
    portfolio: [],
    favorites: []
  });
  ...
  useEffect(() => {
    if (apps && launchpadApps && portfolioData) {
      setAllApps({
        app: apps,
        portfolio: launchpadApps,
        favorites: portfolioData.websites.favorites.sites.map(site => ({
          id: site.id,
          title: site.title,
          img: site.img,
          link: site.link
  ...
  }, [apps, launchpadApps, portfolioData]);
