...
const NavPage = ({ width, setGoURL }: NavProps) => {
  ...
  if (loading) return (
    <div className="w-full safari-content flex-center">
      <div>Loading...</div>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="w-full min-h-full pt-8 c-bg-100/80 backdrop-blur-2xl">
        {/* Favorites */}
        <NavSection
          section={data?.websites.favorites || { title: '', sites: [] }}
          setGoURL={setGoURL}
          width={width}
        />
        ...
        {/* Frequently Visited */}
        <NavSection 
          section={data?.websites.freq || { title: '', sites: [] }}
          setGoURL={setGoURL} 
          width={width}
        />
